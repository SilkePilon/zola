...
- [x] **INFO**: Files accessing message.content:
  ...
  (Note: multiple occurrences per file; all will need refactoring to `message.parts`.)
...
- [x] **ACTION**: Run `npm install ai@latest`
- [x] **ACTION**: Verify version in package.json shows ^5.0.0 or higher
- [x] **INFO**: New version installed: 5.0.88

### 2.2 Update Provider & UI Packages (if used)
- [x] **ACTION**: Check package.json for these packages and update if present:
  - `npm install @ai-sdk/openai@latest @ai-sdk/anthropic@latest @ai-sdk/google@latest @ai-sdk/mistral@latest @ai-sdk/perplexity@latest @ai-sdk/xai@latest`
  - `npm install @ai-sdk/react@latest`
  - `npm install @openrouter/ai-sdk-provider@latest`

### 2.3 Update Other Dependencies
- [x] **ACTION**: Update zod: `npm install zod@latest` (>= 3.25.76 or >= 4.1.8 is acceptable)
...
### 2.5 Commit Changes
- [ ] **ACTION**: Commit package updates: `git add package.json package-lock.json && git commit -m "Update to AI SDK 5"`
...
- [ ] **ACTION**: Commit codemod changes: `git add -A && git commit -m "Apply AI SDK 5 codemods"` (deferred per user)
...
- [x] **ACTION**: Create a plan for addressing each FIXME in Phase 4
  ...
  Files with `FIXME(@ai-sdk-upgrade-v5)`:
  - `lib/chat-store/messages/api.ts` (2)
  - `app/types/database.types.ts` (3)
  - `app/components/multi-chat/multi-conversation.tsx` (2)
  - `app/components/chat/get-sources.ts` (2)
  - `app/components/chat/conversation.tsx` (1)
  - `app/api/chat/route.ts` (1)
  - `app/api/chat/api.ts` (1)
...
- [x] **INFO**: Files updated:
  - `app/components/chat/conversation.tsx`
  - `app/components/multi-chat/multi-conversation.tsx`
  - `app/components/multi-chat/multi-chat.tsx` (grouping keys)
  - `app/share/[chatId]/article.tsx`
