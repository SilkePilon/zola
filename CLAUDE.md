...
Auth is Better Auth (`lib/auth.ts` server instance, `lib/auth-client.ts` browser client), configured with email/password, optional Google and GitHub OAuth, and the `anonymous()` plugin for guest sessions. Email/password is always enabled (verification is off — no mail sender is configured); each social provider is registered only when both of its env vars (`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`) are present. `lib/auth-providers.ts` (`getAuthProviders()`, server-only) reads those same vars so the UI only offers configured providers — server components call it directly, client components read it through `GET /api/auth-providers` via the `useAuthProviders()` hook. Values shared with client code (`MIN_PASSWORD_LENGTH`, the `AuthProviders` type) live in `lib/auth-shared.ts`; keep `lib/auth.ts` and `lib/auth-providers.ts` in sync when adding a provider. Sign-in/sign-up UI is composed from `app/components/auth/` (`auth-form.tsx`, `social-auth-buttons.tsx`). `app/api/auth/[...all]/route.ts` is Better Auth's catch-all route handler. A `databaseHooks.user.create.after` hook keeps the app's own `users` profile table in sync whenever Better Auth creates a user (OAuth sign-in or anonymous guest); `onLinkAccount` reassigns a guest's data (`lib/db/reassign-user.ts`) when they later sign in with Google. Session reads use `auth.api.getSession({ headers })` server-side and `authClient`/`useSession()` client-side — there is no `isSupabaseEnabled`-style graceful degradation; Postgres and Better Auth are hard requirements for this app to run.
